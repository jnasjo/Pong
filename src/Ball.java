...
public class Ball {
	 ...
	 * action
	 *
	 * @return first index: -1 if ball went over the left edge, 1 if it went
	 *         over the right and 0 if neither. Second index: 1 if changes have
	 *         been made to the velocity, 0 if not.
	 */
	public int[] moveBall() {
		// If a point should be added
		int res = 0;
		// If the ball got a new velocity/speed
		double oldVX = velX, oldVY = velY;
		...
		return new int[] { res, (oldVX == velX && oldVY == velY) ? 0 : 1 };
	}
